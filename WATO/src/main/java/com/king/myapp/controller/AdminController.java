...
public class AdminController {
	...
	public ModelAndView mailSending(TeachVO vo, HttpServletRequest request, String e_mail,
		...
		String content =
				...
				System.getProperty("line.separator") + // 한줄씩 줄간격을 두기위해 작성
						...
						"http://localhost:8080/teach/teach_join_2?terms1=on&terms2=on&terms3=on" + " 인증번호는 " + dice
						+ " 입니다. " + "http://localhost:8080/teach/teach_join_2?terms1=on&terms2=on&terms3=on" +
	...
	// 이메일로 받은 인증번호를 입력하고 전송 버튼을 누르면 맵핑되는 메소드. //내가 입력한 인증번호와 메일로 입력한 인증번호가 맞는지 확인해서
	// 맞으면 회원가입 페이지로 넘어가고, //틀리면 다시 원래 페이지로 돌아오는 메소드
	...
	public ModelAndView join_injeung(String email_injeung, @PathVariable String dice,
		...
		mv.setViewName("/teach/teach_join");
	...
	// 아이디&비밀번호 찾기 POST(이메일 전송)
	@RequestMapping(value = "/forgot_id_pwd", method = RequestMethod.POST)
	public ModelAndView postFgid(TeachVO tvo, Model model, HttpServletRequest request, HttpServletResponse response_email) throws Exception {
		...
		TeachVO list = adminservice.findidpwd(tvo);
		...
		if (list.getTeach_Email().equals(tvo.getTeach_Email())) {
			
			tvo.setUser_Pwd(pw);
			
			adminservice.changepwd(tvo);
